...
| Запрет install-скриптов | `ignore-scripts` в конфиге пакетного менеджера | обсуждается вместе с карантином версий: карантин задерживает свежую версию, запрет скриптов убирает исполнение кода при установке |
