...
**Пока в копии работает хоть один исполнитель — коммить только перечисленными
путями** (`git add <файл> <файл>`). `git add -A`, `git commit -a`, `git checkout .`,
`git stash`, `git clean` запрещены: копия у блока одна, и эти команды забирают или
сносят незакоммиченную работу исполнителя. Отдавая задачу, назови ему файлы,
которыми он владеет, и сам в них не коммить. Проверено на живом прогоне: чужой
файл уехал в коммит с посторонним сообщением, а каталог переводов — в
промежуточном, нерабочем состоянии. Хуже всего попадает намеренная порча, которой
исполнитель проверяет свою же проверку: закоммиченная и не откатанная, она уже не
шум, а дефект.
