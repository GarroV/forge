...
<!-- Обязательный минимум для этого проекта: TDD (красный → зелёный → рефактор) для новой функциональности; порог покрытия — не ниже 80%. Какие типы тестов обязательны (unit / integration / e2e) и где граница критичного пути, который нельзя сдавать без теста. -->
...
<!-- Секреты никогда не попадают в git: значения — в .env (gitignored), в документации — только имена переменных. Валидация пользовательского ввода на границах системы. Что ещё специфично для этого продукта (аутентификация, платежи, персональные данные пользователей) — перечислить явно. -->
