...
4. **Одновременно с флагом работает ровно одна сессия.** Канал слушает бота
   long-polling'ом, а Telegram допускает только одного слушателя на токен: вторая
   сессия с флагом **выбивает первую** ответом `409 Conflict` — и выбитая при этом
   не падает, а просто перестаёт получать сообщения. Владелец видит бота живым и
   считает, что канал в порядке, хотя стройка ждёт ответа, который до неё не
   дойдёт. Проверено экспериментом, а не выведено из документации.
5. **`telegram: on` в личном профиле** — иначе система сознательно не отправляет
