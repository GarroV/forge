...
# Секция E: все восемь подсекций доступов на месте.
for sub in "E1. GitHub" "E2. Целевая площадка" "E3. База данных" "E4. Домены" \
           "E5. Сторонние API" "E6. Платежи" "E7. Аналитика" "E8. Существующие данные"; do
  grep -qF "### $sub" "$INTAKE" || {
...
(( access_tables == 8 )) || {
  echo "FAIL: intake.md — таблиц доступов $access_tables, ожидалось 8 (по подсекции на каждую)"
  exit 1
}
