...
check_fixture() {
  ...
  if [[ -z "$marker_line" ]]; then
    ...
    while IFS= read -r block; do
      ...
      # называет в ней состав волны («Запущен блок core (T004-T008): роль …»).
      # Требование двоеточия сразу после имени объявляло такую запись отсутствующей и
  ...
  while IFS= read -r drow; do
    ...
    if [[ "$dcols" != "5" ]]; then
      ...
      echo "      Формат объявлен неизменным — его парсят forge-build и cursus."
      exit 1
    fi
