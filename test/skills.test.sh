...
for pair in "forge-new|отчёта прогона и путь к нему" "forge-quality-gates|формат машинного отчёта прогона"; do
  IFS='|' read -r skill_name marker <<< "$pair"
  target="$SKILLS_DIR/$skill_name/SKILL.md"
  [[ -f "$target" ]] || continue
  grep -qF -- "$marker" "$target" || \
    fail "[$skill_name] не называет машинный отчёт прогона — приёмке будет нечего читать"
done
